  ...
  return (
    <div>
      ...
      {data.temPosicionamento ? (
        ...
        <div className="mb-12 rounded-2xl bg-brand-dark p-6 text-bone">
      ...
      <div>
        ...
        {data.formatos.length > 0 || editable ? (
          <ul className="list-inside list-disc space-y-1.5 text-[16px] leading-[1.65] marker:text-brand">
      ...
      {data.linhas.length > 0 && (
        <div className="mt-8">
          {data.linhas.map((linha, i) => (
            <div key={i} className="mb-4 rounded-2xl border border-brand bg-bone p-6 last:mb-0">
              <div className="mb-4 flex items-center gap-3">
                <span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-brand font-mono text-sm font-bold text-bone">
                  {i + 1}
                </span>
                {editable && onChange ? (
                  <EditableText
